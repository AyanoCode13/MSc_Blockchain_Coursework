...
export default async function Home({
  ...
  for (const [key, value] of Object.entries(searchParams)) {
    if (value !== null) {
      if (key === "page") {
        ...
        nextLink += `${key}=${value as string}&`;
        prevLink += `${key}=${value as string}&`;
      }
    }
  }
  // console.log("link");
  ...
  const properties = await api.property.getByFilter({
    ...
    property_type:searchParams.property_type ? searchParams.propertyType as "House" | "Apartment" : undefined,
